...
class PASHA(MinimizersImpl):
    ...
    def __init__(self, w, k):
        ...
        self.fn = f'./pasha_uhs/PASHA{k}_{self.pasha_uhs[k]}.txt'
        self.decyc_fn = f'./pasha_uhs/decyc{k}.txt'
        if not os.path.exists('./pasha_uhs'):
            os.makedirs('./pasha_uhs')
        request.urlretrieve(self.url, self.fn)
        request.urlretrieve(self.decyc_url, self.decyc_fn)
    ...
    def kmer_level(self, kmer):
        return (self.data[kmer // self.bpd] & (2 ** (kmer % self.bpd))) > 0
